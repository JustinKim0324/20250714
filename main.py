...
def main():
    ...
    st.markdown("""
    이 앱을 만든 이유: <br><br>
    올해 6월 10일부터 KRX(한국 거래소)에서 K200 야간선물 거래를 런칭하였습니다. 이제는 기존 EUREX가 아닌 KRX에서 관장하기 때문에<br>
    야간선물 시장에서도 투자자별 동향 데이타가 오픈이 됩니다. 야간선물 시장은 미국장이 열리는 시간대이기 때문에 미국 증시 움직임의 영향을 많이 받습니다.<br>
    간밤의 야간선물 시장에서의 외국인 동향이 다음날 우리 증시 정규장에서 외국인 현,선물 동향과 주로 일치된 방향성을 보일 것인지 여부를 추적해보기 위하여 만들어보았습니다.
    """)
